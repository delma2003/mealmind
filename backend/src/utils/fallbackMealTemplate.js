"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const fallbackMealTemplate = `
7-Day Balanced Meal Plan (2000 kcal/day)

Day 1:
- Breakfast: Oatmeal with banana & almonds
- Lunch: Grilled chicken salad with olive oil
- Dinner: Quinoa with steamed vegetables & tofu

Day 2:
- Breakfast: Greek yogurt with berries
- Lunch: Brown rice with lentil curry
- Dinner: Baked salmon with roasted broccoli

Day 3:
- Breakfast: Smoothie (spinach, banana, protein powder)
- Lunch: Chicken wrap with hummus
- Dinner: Whole wheat pasta with tomato basil sauce

Day 4:
- Breakfast: Scrambled eggs & toast
- Lunch: Chickpea & avocado bowl
- Dinner: Grilled paneer with stir-fried vegetables

Day 5:
- Breakfast: Chia pudding with mango
- Lunch: Tuna sandwich with lettuce & cucumber
- Dinner: Mushroom risotto

Day 6:
- Breakfast: Muesli with soy milk
- Lunch: Rice with rajma (kidney beans)
- Dinner: Chicken stir-fry with bell peppers

Day 7:
- Breakfast: Boiled eggs & fruits
- Lunch: Tofu rice bowl
- Dinner: Grilled shrimp with couscous

This is a general healthy meal plan to suit most needs. Please consult a dietitian for medical advice.
`;
exports.default = fallbackMealTemplate;
